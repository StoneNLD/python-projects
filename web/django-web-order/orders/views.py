...
from .models import Category, RegularPizza, SicilianPizza, Toppings, Sub, Pasta, Kaasplankje, Salad, DinnerPlatters, UserOrder, SavedCarts
...
def kaasplankje(request):
    if request.user.is_authenticated:
        return render(request, "orders/kaasplankje.html", context = {"dishes":Kaasplankje.objects.all})
    else:
        return redirect("orders:login")
